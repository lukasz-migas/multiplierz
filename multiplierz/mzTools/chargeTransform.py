...
def stripNone(sequence):
    ...
    return list(dropwhile(lambda x: x[0] is None, sequence))
...
def assignMassCharge(peakPoints, mass):
    ...
    noneGapFraction = len([x for x in stripPeaks if x[0] is None]) / float(len(stripPeaks))
    if noneGapFraction > 0.3:
        truePeaks = [x for x in stripPeaks if x[0] is not None]
        ...
        for multiple in [2,3,5,7,11]:
            ...
            nextGapFraction = len([x for x in nextPeaks if x[0] is None]) / float(len(nextPeaks))
...
def zoomOnMostIntense(spectrum, maxLabels, label):
    ...
    if maxLabels is None: maxLabels = 99

...
class XLSXReader(XLSheetReader):
    def __init__(self, file_name, sheet_name=None):
        ...
        try:
            self.columns = [x.value if x.value is not None else '' for x in next(self.sheet.iter_rows()) if x]
    ...
    def __iter__(self):
        ...
        for row in iterator:
            values = [x.value if x.value is not None else '' for x in row]
            if not any(values):
                continue
...
class XLSXWriter(XLSheetReader):
    def __init__(self, file_name, sheet_name='Data', columns=None,
        ...
        if not columns:
            raise IOError("No columns!  Also, use XLSheetReader instead of XLSXWriter directly.")
...
class XLSWriter(XLSheetWriter):
    def __init__(self, file_name, sheet_name='Data', columns=None,
                 overwrite_corrupt=default_overwrite_corrupt, **etc):
        if not columns:
            raise IOError("No columns!  Also, use XLSheetReader instead of XLSWriter directly.")
    ...
    def write(self, row, metadata=None, ignore_extra=default_ignore_extra_columns):
        ...
        for index, value in enumerate(row):
            self.sheet.write(self.currentRow, index, value if (
                isinstance(value, Number) or value is None) else str(value))
        self.currentRow += 1
...
class XLSReader(XLSheetWriter):
    def __init__(self, file_name, sheet_name=None):
        ...
        self.columns = [x.value if x.value is not None else '' for x in self.sheet.row(0)]
